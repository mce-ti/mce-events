...
const useProductMovement = ({ navigation, route: { params }, showAlert }: useProductMovementPrps) => {
  ...
  const estoqueLimpo = stockInfos.estoque_limpo[params.indice_estoque];
  ...
  const getQuantity = (id_art : number) => {
    const subStockItem = estoqueLimpo.find(item => item.id_arte == id_art);

    return subStockItem?.quantidade || 0;
  }
